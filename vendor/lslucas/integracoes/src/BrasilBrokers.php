...
class BrasilBrokers {
    ...
    public $storage_id = null, $data = null, $integracao = 'BrasilBrokers', $importPhotos = true, $estate_id = null;
    ...
    public function parse($storage_id=null)
    {
        ...
        $parsed = @$Parser->xml(utf8_encode($xml))['Imoveis'];
    ...
    public function format($payload)
    {
        ...
        foreach ( $payload as $imoveis ) {
            ...
            foreach ( $imoveis as $item ) {
                ...
                $bairro = isset($item['Bairro']) ? ' no '.$item['Bairro'] : null;
                $titulo = $item['TipoImovel'].$bairro.', '.$item['Cidade'];
                ...
                $formated[$i]['titulo'] = Encoding::fixUTF8($titulo);
                $formated[$i]['acao'] = $acao;
                $formated[$i]['tipo'] = 'mude_ja';
                $formated[$i]['imovel'] = @$item['TipoImovel'];
                $formated[$i]['endereco'] = Encoding::fixUTF8(@$item['Endereco']);
                $formated[$i]['cidade'] = Encoding::fixUTF8(@$item['Cidade']);
                $formated[$i]['bairro'] = Encoding::fixUTF8(@$item['Bairro']);
                $formated[$i]['cep'] = @$item['CEP'];
                $formated[$i]['numero'] = @$item['Numero'];
                $formated[$i]['valor'] = $valor;
                $formated[$i]['valor_condominio'] = $valorCondominio;
                $formated[$i]['area_util_m2'] = isset($item['AreaUtil']) ? $Numbers::decimalize($item['AreaUtil']) : null;
                $formated[$i]['area_total_m2'] = isset($item['AreaTotal']) ? $Numbers::decimalize($item['AreaTotal']) : null;
                $formated[$i]['obs'] = Encoding::fixUTF8(@$item['Observacao']);
        ...
        foreach ( $this->data as $item ) {
            ...
            if ( isset($item['fotos']) && $this->importPhotos ) {
                foreach ( $item['fotos'] as $foto ) {
                    ...
                    $Upload->mocking(trim($foto), ['area' => 'realestate', 'token' => $item['token']]);
                }
            }
